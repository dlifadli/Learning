...
$id = $_GET['id'];
$store = query("SELECT tb_store.* FROM tb_store
JOIN tb_seller ON tb_store.id_seller = tb_seller.id_seller
WHERE tb_seller.id_seller IN (
  SELECT id_seller FROM tb_store
	WHERE id_seller = $id
  GROUP BY id_seller
  HAVING COUNT(*) > 0
);");
    ...
    <div class="pilih">
      ...
      <script>
        ...
        select.addEventListener('change', () => {
          window.location.href = `tambah-input-pd.php?id=${select.value}`;
        });
      </script>
    </div>
